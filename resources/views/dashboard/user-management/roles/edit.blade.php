...
@section('title', 'Update ' . $pageTitle)
@section('body')
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="d-flex flex-column-fluid">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ route($routePrefix . 'update', $role->slug) }}" method="POST" autocomplete="off">
                            <div class="card card-custom gutter-b card-stretch card-shadowless">
                                <div class="card-body">
                                    {!! \App\Helpers\Input::textField('name', 'Name', TRUE, '', $role->name) !!}
                                                                ...
                                                                @foreach($module->permission as $permission)
                                                                    @php
                                                                        ...
                                                                        if(!empty(old('permissions')) && in_array($permission->id, old('permissions'))) {
                                                                            $is_checked = 'checked';
                                                                        } elseif(in_array($permission->id, array_column($role->permission->toArray(), 'id'))) {
                                                                            $is_checked = 'checked';
                                                                        }
                                                                    @endphp
                                                                    <td>
